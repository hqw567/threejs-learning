...
startDom.innerHTML = '点击屏幕开始!'
document.body.appendChild(startDom)
...
const config = {
  ...
  dogNearbyThreshold: 1,
  dog1CameraFollow: false,
  LightType: 'Point',
}
// GUI 控制
const gui = new GUI()
...
const dogmaterial = new THREE.MeshBasicMaterial({ color: 0x8b4513 })
//Lights
// 添加环境光
...
for (const lamp of lamps) {
  // 添加点光源
  const light1 = new THREE.PointLight(0xffff00, 1, 10, 2)
  light1.position.set(0, 1 + 0.2, 0) // 设置光源位置在灯头处
  lamp.add(light1) // 添加到组中
  pointLights.push(light1)

  // 添加方向光源
  const light2 = new THREE.DirectionalLight(0xffffff, 1)
  light2.position.set(0, 1 + 0.2, 0) // 设置光源位置在灯头处
  lamp.add(light2)
  light2.visible = false
  directionalLights.push(light2)
}

gui.add(config, 'LightType', ['Directional', 'Point']).onChange((value) => {
  for (let i = 0; i < lamps.length; i++) {
    if (value === 'Directional') {
      directionalLights[i].visible = true
      pointLights[i].visible = false
    } else if (value === 'Point') {
      directionalLights[i].visible = false
...
gui.add(config, 'dogNearbyThreshold', 0.1, 5, 0.01)
gui.add(config, 'dog1CameraFollow')

function checkIfDogsAreNearby(dog1, dog2) {
  if (dog1 && dog2) {
    const distance = dog1.position.distanceTo(dog2.position)
    if (distance < config.dogNearbyThreshold) {
      dogmaterial.color.set('red')
      dogBarking.on()
    } else {
      dogmaterial.color.set('#8b4513')
    }
  }
}
...
// 动画函数，用来移动相机
const targetPosition2 = new THREE.Vector3(3, 5.5, 6)
// 动画循环
function animate() {
  ...
  // 检查是否允许动画
  if (animationAllowed && startDom.style.display === 'none') {
    camera.position.lerp(targetPosition2, 0.05)
    camera.lookAt(scene.position)

    // 检查相机是否接近目标位置
    if (camera.position.distanceTo(targetPosition2) < 0.01) {
      animationAllowed = false // 停止动画
      camera.position.copy(targetPosition2) // 确保相机精确到达目标位置
    }
  }
  // 计算跳跃
  ...
  if (dog1 && config.dog1CameraFollow) {
    camera.position.copy(dog1.position)

    camera.lookAt(dog1.position)
  }
  checkIfDogsAreNearby(dog1, dog2)
...
function createDog() {
  ...
  // 创建狗的头部
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.05, 32, 32), dogmaterial)
  ...
  // 创建狗的身体
  const body = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.1, 0.1), dogmaterial)
  ...
  // 创建狗的尾巴
  const tail = new THREE.Mesh(new THREE.SphereGeometry(0.02, 32, 32), dogmaterial)
